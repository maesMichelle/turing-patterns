...
void main() {
  gl_Position = projectionMatrix * viewMatrix * modelMatrix * vec4(position, 0.7);
  vUv = uv;
}
`;
...
void main() {
  ...
  // float gray = (originalColor.r + originalColor.g + originalColor.b) / 3.0;
   //float gray = 0.09 * originalColor.r + 0.8 * originalColor.g + 0.33 * originalColor.b;
  gray = clamp(gray, 0.0, 1.0);
  gl_FragColor = texture2D(uColorTexture, vec2(gray, 01.0));
